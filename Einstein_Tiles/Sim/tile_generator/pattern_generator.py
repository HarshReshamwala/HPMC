...
def build_supertiles(target_level):
    global tiles
    global level

    # Build up to the specified target level
    while level < target_level:
        patch = construct_patch(tiles[0], tiles[1], tiles[2], tiles[3])
        tiles = construct_metatiles(patch)
        level += 1

def draw(target_level):
    global tiles

    # Check if target_level is achieved in the tiles list
    if target_level <= level:
        tiles[0].draw(to_screen_mat, target_level)
    else:
        print("The target level has not been built yet.")
